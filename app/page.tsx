...
import { useState, useRef } from "react" // 增加 useRef
...
import { Copy, ImageDown } from "lucide-react" // 引入图标
import { toPng } from 'html-to-image' // 引入图片转换库
  ...
  const handleExportImage = async () => {
    ...
    try {
      const dataUrl = await toPng(cardRef.current, { 
        ...
        cacheBust: true,
        style: { borderRadius: '0' } // 导出时稍微调整样式确保完整性
      });
      ...
      toast.success("墨宝装裱完成，已保存至本地");
    } catch (err) {
      toast.dismiss(loadingToast);
      toast.error("装裱失败，请重试");
    }
  };
    ...
    <main className="min-h-screen bg-stone-50 py-10 px-4 md:px-20 font-serif text-stone-900">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          {/* 修改点：md:sticky 确保只在桌面端固定，修复移动端遮挡问题 */}
          <Card className="border-stone-200 shadow-sm md:sticky md:top-10 z-10">
            <CardHeader>
            ...
            <CardContent className="space-y-4">
              <Textarea 
                ...
                className="min-h-[200px] border-stone-200 focus-visible:ring-stone-400 bg-white/50 font-sans"
                value={inputText}
              ...
              <Button onClick={handleTranslate} disabled={isLoading} className="w-full bg-stone-800 hover:bg-stone-700 text-stone-100 py-6 text-lg font-sans">
          ...
          {/* 结果卡片：增加功能按钮 */}
          <Card ref={cardRef} className="border-stone-300 bg-[#fbfaf5] shadow-md relative min-h-[450px] flex flex-col border-l-4 border-l-stone-400 overflow-hidden">
            ...
            {result && !isLoading && (
              <div className="absolute top-4 right-4 flex gap-2 z-20 print:hidden">
                <Button variant="ghost" size="icon" onClick={handleCopy} title="复制墨宝" className="h-8 w-8 text-stone-400 hover:text-[#8e292c] hover:bg-white/50 transition-colors">
                  <Copy className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" onClick={handleExportImage} title="装裱保存" className="h-8 w-8 text-stone-400 hover:text-[#8e292c] hover:bg-white/50 transition-colors">
            ...
            <CardContent className="flex-grow flex flex-col p-8 pt-12 relative group">
              {!result && !isLoading ? (
                ...
                <div className="flex flex-col h-full animate-in fade-in duration-1000">
                  ...
                  <div className="mt-12 pt-6 border-t border-stone-100 relative">
